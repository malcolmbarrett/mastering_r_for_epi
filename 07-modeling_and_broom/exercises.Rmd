...
_Bonus_: Add a regresssion line with geom_smooth(method = "lm"). Remember that with ggplot2, you add functions with `+` and not `%>%`
...
Diabetes is frequently defined as an A1c value of 7.0 or greater. Create a binary variable (0, 1) called `diabetic` indicating diabetes. Use the `ifelse` function. Use ?ifelse if you're  not sure how to use this function.
...
diabetes %>% 
  mutate(diabetic = ifelse(glyhb >= 7.0, ___, ___)) %>% 
  _______ %>% 
  _______
```
